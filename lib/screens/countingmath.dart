...
class _CountingActivityScreenState extends State<CountingActivityScreen>
  ...
  String getInstructionText() {
    if (currentLevel == 3) return "Drag the correct number to the objects";
    switch (currentLevel) {
      case 1: return "Look at the objects. How many?";
      case 2: return "Count the different objects. How many in total?";
          ...
          children: [
            AnimatedScale(
              ...
              child: Image.asset(
                ...
                errorBuilder: (_, __, ___) => Container(
                  ...
                  child: Icon(Icons.image_not_supported, color: Colors.grey, size: 32.w),
                ),
              ),
            ),
            if (isHighlighted && isCountingHint)
              Container(
                ...
                child: Center(
                  child: Text("${index + 1}", style: GoogleFonts.fredoka(fontSize: 32.sp, fontWeight: FontWeight.bold, color: Colors.white)),
                ),
              ),
          ],
        );
  ...
  Widget buildAnswerChoices() {
          ...
          return Draggable<int>(
            ...
            feedback: Material(
              ...
              child: Container(
                ...
                child: Center(child: Text(num.toString(), style: GoogleFonts.fredoka(fontSize: 50.sp, fontWeight: FontWeight.bold))),
              ),
            ),
            ...
            child: Container(
              ...
              child: Center(child: Text(num.toString(), style: GoogleFonts.fredoka(fontSize: 48.sp, fontWeight: FontWeight.bold))),
            ),
          );
        }).toList(),
      );
    ...
    return Row(
      ...
      children: choices.map((num) {
        ...
        final isWrongChoice = hasAnswered && num != targetCount && showPrompt;
    ...
    return DragTarget<int>(
      onAccept: (data) {
        if (data == targetCount) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Try again!"), duration: Duration(milliseconds: 800)),
          );
        }
      },
      builder: (context, candidateData, rejectedData) {
        return Container(
          ...
          height: 300.h,
          decoration: BoxDecoration(
            border: Border.all(color: candidateData.isNotEmpty ? Colors.green : Colors.grey, width: 4),
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      onPopInvoked: (didPop) async {
        if (didPop) return;
        widget.onSessionComplete(); // Direct exit to child's dashboard
      },
      child: Scaffold(
        body: SafeArea(
            ...
            child: Column(
              children: [
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const Spacer(),
                      Text('Counting', style: GoogleFonts.fredoka(fontSize: 26.sp, fontWeight: FontWeight.w500, color: colorScheme.primary)),
                      const Spacer(),
                ...
                if (currentLevel == 3) ...[
                  ...
                  Text(getInstructionText(), style: GoogleFonts.fredoka(fontSize: 24.sp, color: colorScheme.primary)),
                  SizedBox(height: 20.h),
                  buildAnswerChoices(),
                ] else ...[
                  Card(
